...
public class InformixBulkUpsertExecutor implements BulkUpsertExecutor {
	private final Dialect dialect;

	public InformixBulkUpsertExecutor(final Dialect dialect) {
	...
	private String stageName(final BulkUpsertOption option) {
		...
		if (!name.matches("[A-Za-z][A-Za-z0-9_]{0,126}"))
			throw new IllegalArgumentException("Invalid Informix stagingTableName: " + name);
		return name;
	}
