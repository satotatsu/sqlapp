...
public class SqlServerBulkUpsertExecutor implements BulkUpsertExecutor {
	...
	public long execute(final Connection connection, final Table table, final BulkUpsertOption options)
		...
		try (var scope = BulkUpsertExecutionScope.begin(connection, effective.isUseTransaction())) {
			try {
				...
				final long affected = executeMerge(connection, table, targetName, stagingName, keys, stagingColumns,
						updateColumns, effective);
				scope.commit();
				return affected;
	...
	private long executeMerge(final Connection connection, final Table table, final String targetName,
			final String stagingName, final List<Column> keys, final List<Column> stagingColumns,
			final List<Column> updateColumns, final BulkUpsertOption options) throws SQLException {
		...
		try (var statement = connection.createStatement()) {
			...
			return statement.executeUpdate(
					createMergeSql(targetName, stagingName, keys, stagingColumns, updateColumns, options));
	...
	private String createStagingSql(final String targetName, final String stagingName, final List<Column> columns) {
		...
		return "SELECT TOP (0) " + columnList + " INTO " + quote(stagingName) + " FROM " + targetName
				+ " UNION ALL SELECT TOP (0) " + columnList + " FROM " + targetName;
	}

	private String createMergeSql(final String targetName, final String stagingName, final List<Column> keys,
			final List<Column> stagingColumns, final List<Column> updateColumns, final BulkUpsertOption options) {
		final StringBuilder sql = new StringBuilder("MERGE INTO ").append(targetName)
				.append(" WITH (HOLDLOCK) AS target USING ").append(quote(stagingName)).append(" AS source ON ");
		for (int i = 0; i < keys.size(); i++) {
			if (i > 0) {
				sql.append(" AND ");
			}
			sql.append("target.").append(quote(keys.get(i).getName())).append(" = source.")
					.append(quote(keys.get(i).getName()));
		}
		...
		if (options.isInsertWhenNotMatched()) {
			sql.append(" WHEN NOT MATCHED BY TARGET THEN INSERT (").append(columnList(stagingColumns, null))
					.append(") VALUES (").append(columnList(stagingColumns, "source")).append(')');
		}
	...
	private BulkOption stagingBulkOption(final BulkOption source) {
		...
		return BulkOption.builder().batchSize(option.getBatchSize()).bulkCopyTimeout(option.getBulkCopyTimeout())
				.keepIdentity(true).keepNulls(true).tableLock(option.isTableLock()).build();
	}
	...
	private String stagingName(final BulkUpsertOption options) {
		...
		if (!name.matches("#[A-Za-z0-9_]+")) {
			throw new IllegalArgumentException("SQL Server stagingTableName must be a local temp table: " + name);
		}
		return name;
	}
