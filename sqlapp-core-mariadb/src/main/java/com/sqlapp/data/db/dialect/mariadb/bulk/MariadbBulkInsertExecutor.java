...
public class MariadbBulkInsertExecutor implements BulkInsertExecutor {
	...
	public long execute(final Connection connection, final Table table, final BulkOption options) throws SQLException {
		...
		try (MySqlBulkDataInputStream input = new MySqlBulkDataInputStream(table, options);
			...
			final org.mariadb.jdbc.Statement mariadbStatement = statement.unwrap(org.mariadb.jdbc.Statement.class);
			final StringBuilder sql = new StringBuilder("LOAD DATA LOCAL INFILE " + "'sqlapp-stream' INTO TABLE ")
			...
			for (int i = 0; i < input.getColumns().size(); i++) {
				final var column = input.getColumns().get(i);
				if (column.getDataType() != null && column.getDataType().isBinary()) {
					sql.append(firstBinary ? " SET " : ", ").append(dialect.quote(column.getName()))
			...
			throw new SQLException("Failed to stream MariaDB bulk rows for " + table.getName(), e);
		}
	}
	...
	private void validateOptions(final BulkOption options) {
		if (options != null && (options.getBatchSize() != null || options.getBulkCopyTimeout() != null
				|| options.isAllowEncryptedValueModifications() || options.isCheckConstraints()
				|| options.isFireTriggers() || options.isKeepNulls() || options.isTableLock()
				|| options.isUseTransaction())) {
			throw new IllegalArgumentException("MariaDB LOAD DATA does not support the requested bulk options");
		}
	}
}
