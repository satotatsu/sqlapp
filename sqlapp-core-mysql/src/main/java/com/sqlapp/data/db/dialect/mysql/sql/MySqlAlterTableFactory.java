...
public class MySqlAlterTableFactory extends AbstractAlterTableFactory<MySqlSqlBuilder> {
	...
	public List<SqlOperation> createDiffSql(final DbObjectDifference difference) {
		final Map<String, Difference<?>> allDiff = difference.toDifference().getChangedProperties(this.getDialect());
		...
		if (consDiff != null) {
			final List<SqlOperation> commands = getAddForeignKeyConstraintDefinitions(originalTable, table, difference,
					consDiff);
	...
	protected void addTableOptions(final Map<String, Difference<?>> allDiff, final MySqlSqlBuilder sqlBuilder) {
	 ...
	 * @param sqlBuilder
	 */
	protected void addEnginDefinition(final Difference<?> tableProp, final MySqlSqlBuilder sqlBuilder) {
	 ...
	 * @param sqlBuilder
	 */
	protected void addRowFormatDefinition(final Difference<?> tableProp, final MySqlSqlBuilder sqlBuilder) {
	 ...
	 * @param sqlBuilder
	 */
	protected void addRemarkDefinition(final Difference<?> tableProp, final MySqlSqlBuilder builder) {
		if (tableProp == null) {
			return;
		}
		final String targetProp = tableProp.getTarget(String.class);
	...
	protected void addColumnDefinitions(final DbObjectDifferenceCollection colsDiff,
			final DbObjectDifferenceCollection consDiff, final MySqlSqlBuilder sqlBuilder) {
	...
	private void addDropForeignKeys(final List<ForeignKeyConstraint> fks, final DbObjectDifferenceCollection consDiff,
			final MySqlSqlBuilder sqlBuilder) {
	...
	protected SqlOperation getAutoIncrementStart(final DbObjectDifference difference,
		...
		for (final DbObjectDifference diff : colDiff.getList()) {
			if (diff.getState() == State.Added || diff.getState() == State.Modified) {
				final Column column = diff.getTarget(Column.class);
				...
				if (current != null && !CommonUtils.eq(current, Long.valueOf(1))) {
					...
					sqlBuilder.space().property("AUTO_INCREMENT", current);
					final SqlOperation operation = createOperation(sqlBuilder.toString(), SqlType.ALTER, orgTable,
							table);
					return operation;
				}
			}
	...
	protected List<SqlOperation> getAddForeignKeyConstraintDefinitions(final Table originalTable, final Table table,
		...
		for (final DbObjectDifference diff : consDiff.getList()) {
			...
			if (diff.getState() == State.Modified || diff.getState() == State.Added) {
				if (!(constraint instanceof ForeignKeyConstraint)) {
					continue;
				}
				addConstraintDefinition(originalTable, table, constraint, sqlBuilder);
	 ...
	 */
	@Override
	protected void addConstraintDefinition(final Table table, final Constraint constraint,
			final AddTableObjectDetailFactory<Constraint, AbstractSqlBuilder<?>> sqlFactory,
	...
	@Override
	protected void dropConstraintDefinition(final Constraint constraint, final MySqlSqlBuilder builder) {
	...
	protected void addIndexDefinitions(final Table originalTable, final Table table,
			final DbObjectDifferenceCollection indexDiff, final DbObjectDifferenceCollection consDiff,
			final MySqlSqlBuilder builder) {
		Map<String, DbObjectDifference> consMap;
		if (consDiff != null) {
			consMap = consDiff.toMap(obj -> obj.getOriginal() instanceof UniqueConstraint
					|| obj.getTarget() instanceof UniqueConstraint);
		} else {
	...
	protected void addIndexDefinition(final Index index, final MySqlSqlBuilder builder) {
		...
		final AddTableObjectDetailFactory<Index, MySqlSqlBuilder> indexOperation = this
	...
	protected void addPartitionDefinition(final Table originalTable, final Table table,
			final DbObjectDifference partitioningProp, final MySqlSqlBuilder builder) {
	...
	protected void modifyPartitionByDefinition(final DbObjectDifference partitioningProp,
			final Partitioning originalPartitioning, final Partitioning partitioning, final MySqlSqlBuilder builder) {
	...
	protected void modifyPartitions(final Partitioning originalPartitioning, final Partitioning partitioning,
			final DbObjectDifferenceCollection partitionsDifference, final MySqlSqlBuilder builder) {
	...
	protected void addPartitionsDefinition(final Partitioning partitioning, final List<DbObjectDifference> child,
			final MySqlSqlBuilder builder) {
		final MySqlCreatePartitioningFactory factory = this.getSqlFactoryRegistry().getSqlFactory(partitioning,
				SqlType.CREATE);
		if (factory == null) {
			return;
		}
	...
	protected void deletePartitionsDefinition(final List<DbObjectDifference> child, final MySqlSqlBuilder builder) {
	 ...
	 * @param builder
	 */
	protected void addCharSetDefinition(final Difference<?> charSetProp, final Difference<?> collationProp,
			final MySqlSqlBuilder builder) {
