...
public abstract class AbstractCreateCatalogTest extends AbstractSqlFactoryTest {
	...
	protected void before() {
		createCatalogOperationFactory = createSqlFactoryRegistry().getSqlFactory(new Catalog("scm"), SqlType.CREATE);
	}

	protected String getSqlText(String filename) throws Exception {
		Catalog catalog = SchemaUtils.readXml(getResourceAsInputStream(filename));
