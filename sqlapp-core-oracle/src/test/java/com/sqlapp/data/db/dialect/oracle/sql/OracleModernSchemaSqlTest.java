...
class OracleModernSchemaSqlTest extends AbstractOracleSqlFactoryTest {
	...
	void testCreateAndDropJsonRelationalDualityView() {
		...
		final String createSql = sqlFactoryRegistry.createSql(view, SqlType.CREATE).get(0).getSqlText()
				.replaceAll("\\s+", " ");
		assertTrue(createSql.contains("CREATE JSON RELATIONAL DUALITY VIEW " + "IF NOT EXISTS CUSTOMER_DV AS"),
				createSql);
		assertTrue(createSql.contains("CUSTOMERS @insert @update @delete"), createSql);
		assertTrue(createSql.contains("orders: ORDERS @insert @update"), createSql);

		final String dropSql = sqlFactoryRegistry.createSql(view, SqlType.DROP).get(0).getSqlText().replaceAll("\\s+",
				" ");
		assertTrue(dropSql.contains("DROP VIEW IF EXISTS CUSTOMER_DV"), dropSql);
	}
	...
	void testRejectJsonRelationalDualityViewWithoutStatement() {
		...
		assertThrows(IllegalArgumentException.class, () -> sqlFactoryRegistry.createSql(view, SqlType.CREATE));
	}
	...
	void testNativeJsonBooleanAndVectorTypes() {
		...
		table.getColumns().add(new Column("FLOAT_EMBEDDING").setDataType(DataType.VECTOR).setVectorDimension(768)
				.setVectorElementDataType(DataType.REAL));
		table.getColumns().add(new Column("DOUBLE_EMBEDDING").setDataType(DataType.VECTOR).setVectorDimension(256)
				.setVectorElementDataType(DataType.DOUBLE));
		table.getColumns().add(new Column("FLEXIBLE_EMBEDDING").setDataType(DataType.VECTOR)
				.setVectorElementDataType(DataType.TINYINT));
		table.getColumns().add(new Column("BINARY_EMBEDDING").setDataType(DataType.VECTOR).setVectorDimension(1024)
	...
	void testCreateHnswVectorIndex() {
		...
		OracleAnnotationUtils.setAnnotation(index, "Purpose", "Semantic search");
		...
		assertTrue(
				sql.matches(
						".*PARAMETERS\\s*\\(\\s*type HNSW\\s*,\\s*neighbors 40\\s*,\\s*efconstruction 500\\s*\\).*"),
				sql);
		assertTrue(sql.contains("ANNOTATIONS ( \"Purpose\" 'Semantic search')"), sql);
	}
	...
	void testCreateIvfVectorIndex() {
		final Table table = createVectorTable();
		final Index index = new Index("IDX_DOCUMENTS_EMBEDDING", table.getColumns().get("EMBEDDING"))
				.setIndexType(IndexType.Vector).setVectorDistanceType(VectorDistanceType.EuclideanSquared);
	...
	void testCreateHybridVectorIndex() {
		...
		final Index index = new Index("IDX_DOCUMENTS_HYBRID", table.getColumns().get("CONTENT"))
		...
		final String sql = sqlFactoryRegistry.createSql(index, SqlType.CREATE).get(0).getSqlText().replaceAll("\\s+",
				" ");
		assertTrue(sql.contains("CREATE HYBRID VECTOR INDEX IDX_DOCUMENTS_HYBRID ON DOCUMENTS ( CONTENT )"), sql);
		assertTrue(sql.contains("PARAMETERS ('DATASTORE DOC_DATASTORE " + "FILTER CTXSYS.AUTO_FILTER LEXER DOC_LEXER "
	...
	void testCreateAndDropDataUseCaseDomain() {
		...
		domain.getSpecifics().put(Oracle23aiCreateDomainFactory.CONSTRAINT_NAME, "CK_EMAIL_ADDRESS");
		domain.getSpecifics().put(Oracle23aiCreateDomainFactory.DISPLAY, "LOWER(VALUE)");
		domain.getSpecifics().put(Oracle23aiCreateDomainFactory.ORDER, "LOWER(VALUE)");
		OracleAnnotationUtils.setAnnotation(domain, "Display", "Email address");

		final String createSql = sqlFactoryRegistry.createSql(domain, SqlType.CREATE).get(0).getSqlText()
				.replaceAll("\\s+", " ");
		assertTrue(createSql.contains("CREATE DOMAIN IF NOT EXISTS EMAIL_ADDRESS AS VARCHAR2(320) STRICT"), createSql);
		assertTrue(createSql.contains("DEFAULT ON NULL 'unknown@example.com' NOT NULL"), createSql);
		assertTrue(createSql.contains(
				"CONSTRAINT CK_EMAIL_ADDRESS " + "CHECK (REGEXP_LIKE(VALUE, '^[^@]+@[^@]+$')) " + "INITIALLY DEFERRED"),
				createSql);
		assertTrue(createSql.contains("DISPLAY LOWER(VALUE) ORDER LOWER(VALUE)"), createSql);
		assertTrue(createSql.contains("ANNOTATIONS ( \"Display\" 'Email address')"), createSql);

		final String dropSql = sqlFactoryRegistry.createSql(domain, SqlType.DROP).get(0).getSqlText().replaceAll("\\s+",
				" ");
		assertTrue(dropSql.contains("DROP DOMAIN IF EXISTS EMAIL_ADDRESS"), dropSql);
	}
	...
	void testTableAndColumnAnnotationsAndExistenceClauses() {
		...
		OracleAnnotationUtils.setAnnotation(table, "Display", "Customer table");
		...
		final var operationSql = operations.stream().map(operation -> operation.getSqlText().replaceAll("\\s+", " "))
				.toList();
		final String createSql = operations.get(0).getSqlText().replaceAll("\\s+", " ");
		assertTrue(createSql.contains("CREATE TABLE IF NOT EXISTS CUSTOMERS"), createSql);
		assertTrue(
				operationSql.stream().anyMatch(
						sql -> sql.contains("ALTER TABLE CUSTOMERS ANNOTATIONS " + "( \"Display\" 'Customer table')")),
				operationSql.toString());
		assertTrue(
				operationSql.stream().anyMatch(sql -> sql.contains("ALTER TABLE CUSTOMERS MODIFY ID")
						&& sql.contains("\"Identity\"") && sql.contains("\"Display\" 'Customer ID'")),
				operationSql.toString());
	...
	void testRejectCollectionTypeAsDataUseCaseDomain() {
		...
		assertThrows(IllegalArgumentException.class, () -> sqlFactoryRegistry.createSql(domain, SqlType.CREATE));
	}
	...
	void testRejectVectorBeforeOracle23ai() {
		...
		table.getColumns().add(new Column("EMBEDDING").setDataType(DataType.VECTOR).setVectorDimension(3)
				.setVectorElementDataType(DataType.REAL));
	...
	void testScalableAndSessionSequences() {
		...
		final String scalableSql = sqlFactoryRegistry.createSql(scalable, SqlType.CREATE).get(0).getSqlText()
				.replaceAll("\\s+", " ");
		...
		final String sessionSql = sqlFactoryRegistry.createSql(session, SqlType.CREATE).get(0).getSqlText()
				.replaceAll("\\s+", " ");
	...
	void testSequenceVersionBoundary() {
		...
		final var oracle12 = DialectResolver.getInstance().getDialect("Oracle", 12, 2, 0);
		scalable.setDialect(oracle12);
		final String oracle12Sql = oracle12.createSqlFactoryRegistry().createSql(scalable, SqlType.CREATE).get(0)
				.getSqlText();
		assertTrue(!oracle12Sql.contains("SCALE"), oracle12Sql);

		final var oracle18 = DialectResolver.getInstance().getDialect("Oracle", 18, 0, 0);
		scalable.setDialect(oracle18);
		final String oracle18Sql = oracle18.createSqlFactoryRegistry().createSql(scalable, SqlType.CREATE).get(0)
	...
	void testScalarAndTableSqlMacros() {
		final Function scalar = createSqlMacro("NORMALIZE_CODE", FunctionType.Scalar, "RETURN 'UPPER(code)';");
		final String scalarSql = sqlFactoryRegistry.createSql(scalar, SqlType.CREATE).get(0).getSqlText()
				.replaceAll("\\s+", " ");
		assertTrue(scalarSql.contains("SQL_MACRO(SCALAR)"), scalarSql);

		final Function table = createSqlMacro("ACTIVE_ORDERS", FunctionType.Table,
				"RETURN 'SELECT * FROM ORDERS WHERE ACTIVE = 1';");
		final String tableSql = sqlFactoryRegistry.createSql(table, SqlType.CREATE).get(0).getSqlText()
				.replaceAll("\\s+", " ");
	...
	void testSqlMacroVersionBoundary() {
		final var oracle19 = DialectResolver.getInstance().getDialect("Oracle", 19, 0, 0);
		final Function tableMacro = createSqlMacro("ACTIVE_ORDERS", FunctionType.Table,
				"RETURN 'SELECT * FROM ORDERS WHERE ACTIVE = 1';");
		tableMacro.setDialect(oracle19);
		final String oracle19Sql = oracle19.createSqlFactoryRegistry().createSql(tableMacro, SqlType.CREATE).get(0)
				.getSqlText().replaceAll("\\s+", " ");
		assertTrue(oracle19Sql.contains("SQL_MACRO"), oracle19Sql);
		assertTrue(!oracle19Sql.contains("SQL_MACRO(TABLE)"), oracle19Sql);

		final Function scalarMacro = createSqlMacro("NORMALIZE_CODE", FunctionType.Scalar, "RETURN 'UPPER(code)';");
		scalarMacro.setDialect(oracle19);
		assertThrows(IllegalArgumentException.class,
				() -> oracle19.createSqlFactoryRegistry().createSql(scalarMacro, SqlType.CREATE));

		final var oracle21 = DialectResolver.getInstance().getDialect("Oracle", 21, 0, 0);
		scalarMacro.setDialect(oracle21);
		final String oracle21Sql = oracle21.createSqlFactoryRegistry().createSql(scalarMacro, SqlType.CREATE).get(0)
				.getSqlText().replaceAll("\\s+", " ");
	...
	private Table createVectorTable() {
		final Table table = new Table("DOCUMENTS");
		table.setDialect(dialect);
		table.getColumns().add(new Column("EMBEDDING").setDataType(DataType.VECTOR).setVectorDimension(768)
				.setVectorElementDataType(DataType.REAL));
		return table;
	}

	private Function createSqlMacro(final String name, final FunctionType functionType, final String statement) {
