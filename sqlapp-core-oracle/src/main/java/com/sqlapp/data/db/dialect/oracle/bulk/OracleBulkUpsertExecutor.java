...
public class OracleBulkUpsertExecutor implements BulkUpsertExecutor {
	...
	public long execute(final Connection connection, final Table table, final BulkUpsertOption options)
		...
		try (var scope = BulkUpsertExecutionScope.begin(connection, false)) {
			try (var statement = connection.createStatement()) {
				statement.execute(
						"CREATE GLOBAL TEMPORARY TABLE " + quote(stagingName) + " ON COMMIT PRESERVE ROWS AS SELECT "
								+ columnList(stagingColumns, null) + " FROM " + targetName + " WHERE 1 = 0");
			}
			...
			try (var statement = connection.createStatement()) {
				affected = statement.executeUpdate(
						createMergeSql(targetName, stagingName, keys, stagingColumns, updates, effective));
			}
			return affected;
		}
	}
	...
	private String createMergeSql(final String targetName, final String stagingName, final List<Column> keys,
			final List<Column> stagingColumns, final List<Column> updates, final BulkUpsertOption options) {
		final StringBuilder sql = new StringBuilder("MERGE INTO ").append(targetName).append(" target USING ")
		...
		if (options.isInsertWhenNotMatched()) {
			sql.append(" WHEN NOT MATCHED THEN INSERT (").append(columnList(stagingColumns, null)).append(") VALUES (")
	...
	private String stagingName(final BulkUpsertOption options) {
		...
		if (!name.matches("[A-Za-z][A-Za-z0-9_$#]{0,29}")) {
			throw new IllegalArgumentException("Invalid Oracle stagingTableName: " + name);
		}
		return name;
	}
