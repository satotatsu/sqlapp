...
public final class OracleSetBasedMigrationSnapshotExecutor extends AbstractStagingMigrationSnapshotExecutor {
	public OracleSetBasedMigrationSnapshotExecutor(final Dialect dialect) {
		super(dialect);
	}
	...
	protected String stageName() {
		return "ORA$PTT_" + super.stageName();
	}

	@Override
	protected String createStageSql(final String stage, final String target, final List<String> columns) {
		return "CREATE PRIVATE TEMPORARY TABLE " + stage + " ON COMMIT DROP DEFINITION AS SELECT " + list(columns, "t")
