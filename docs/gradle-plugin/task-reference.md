...
| `verifyMigrationExecutionReport` | `VerifyMigrationExecutionReportTask` | `reportFile`; optional report, plan and database connection fingerprints; `requireSuccessful` and `requireAllSelectedCommitted` default to `false` | Verifies report integrity, approved plan/target identity and optional completion policies | File-only; no database access |
