...
|---|---|---|---|---|
| `assessDatabaseMigration` | `AssessDatabaseMigrationTask` | Required `inputFile`, `targetDatabase`, `targetVersion`, `outputFile`; `failOnBlockers` defaults to `true` | [Database migration assessment](database-migration-assessment.md): Access to Oracle 19c/21c/23ai/26ai or SQL Server 2016/2017/2019/2022; source inventory and target-specific findings as JSON | Offline; Access metadata only, no rows, linked sources or database connections |
...
| `migrationEnvironmentSnapshot` | `MigrationEnvironmentSnapshotTask` | Shared `migration` extension; required `environmentId` and `outputFile` | Writes a fingerprinted snapshot of versioned and repeatable migration state | Reads migration history only |
| `compareMigrationEnvironments` | `CompareMigrationEnvironmentsTask` | At least two `snapshotFiles`; optional `baselineEnvironmentId`, `outputFile`; `failOnDifferences` defaults to `false` | Compares database product/version and migration state; optionally fails as a CI gate | File-only; no database access |
| `verifyMigrationExecutionReport` | `VerifyMigrationExecutionReportTask` | `reportFile`; optional report, plan and database connection fingerprints and `maxReportAgeSeconds`; `requireSuccessful` and `requireAllSelectedCommitted` default to `false` | Verifies report integrity, freshness, approved plan/target identity and optional completion policies | File-only; no database access |
