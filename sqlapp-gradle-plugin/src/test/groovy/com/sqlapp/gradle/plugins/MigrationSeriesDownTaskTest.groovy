...
class MigrationSeriesDownTaskTest extends AbstractTaskTest{
	...
	public void canAddTaskToProject() {
		...
		TaskProvider<MigrationTask> taskProvider =project.tasks.register('migration', MigrationTask)
		MigrationTask task=taskProvider.get();
		DataSource dataSource = getDataSource(extension.dataSource);
		dropTables(dataSource, "TAB1", "TAB2", "changelog");
		task.exec()
		MigrationSeriesDownTask downTask =project.tasks.register('migrationSeriesDown', MigrationSeriesDownTask).get()
		downTask.exec()
	}
}
