...
public final class HsqlSetBasedMigrationSnapshotExecutor extends AbstractStagingMigrationSnapshotExecutor {
	public HsqlSetBasedMigrationSnapshotExecutor(final Dialect dialect) { super(dialect); }
	@Override protected String createStageSql(final String stage, final String target, final List<String> columns) {
		return "DECLARE LOCAL TEMPORARY TABLE " + stage + " AS (SELECT " + list(columns, "t") + " FROM "
				+ target + " t WHERE 1=0) WITH DATA ON COMMIT PRESERVE ROWS";
	}
