...
public class SchemaGraphBuilder extends AbstractSchemaGraphBuilder {
	...
	private List<Table> relationTables(Collection<Table> tables) {
		return tables.stream().filter(table -> this.getDrawOption().getTableFilter().test(table))
				.filter(table -> table.getChildRelations().size() > 0
	...
	private List<Table> partitionTables(Collection<Table> tables) {
		return tables.stream().filter(table -> this.getDrawOption().getTableFilter().test(table))
				.filter(table -> table.getPartitionParent() != null).collect(Collectors.toList());
	}
