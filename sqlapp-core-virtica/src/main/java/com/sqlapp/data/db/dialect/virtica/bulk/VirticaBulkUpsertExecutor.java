...
public class VirticaBulkUpsertExecutor implements BulkUpsertExecutor {
	...
	private List<Column> staged(final Table t, final BulkUpsertOption o, final List<Column> keys) {
	...
	private List<Column> updates(final Table t, final BulkUpsertOption o, final List<Column> keys,
		...
		final Set<String> kn = names(keys), sn = names(staged);
		if (!o.getUpdateColumns().isEmpty()) {
			final List<Column> r = columns(t, o.getUpdateColumns(), "update");
			for (final Column c : r)
				if (kn.contains(c.getName()) || c.isIdentity() || !sn.contains(c.getName()))
