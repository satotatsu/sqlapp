...
public class SapHanaViewColumnReader extends ColumnReader {
	...
	protected Column createColumn(final ExResultSet rs) throws SQLException {
		...
		column.setNullable("TRUE".equalsIgnoreCase(getString(rs, "IS_NULLABLE")));
		getDialect().setDbType(getString(rs, "DATA_TYPE_NAME"), rs.getLong("LENGTH"), getInteger(rs, "SCALE"), column);
