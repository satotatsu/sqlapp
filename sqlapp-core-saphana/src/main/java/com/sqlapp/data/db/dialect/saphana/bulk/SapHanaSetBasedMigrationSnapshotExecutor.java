...
public final class SapHanaSetBasedMigrationSnapshotExecutor extends AbstractStagingMigrationSnapshotExecutor {
	public SapHanaSetBasedMigrationSnapshotExecutor(final Dialect dialect) {
		super(dialect);
	}
	...
	protected String createStageSql(final String stage, final String target, final List<String> columns) {
		return "CREATE LOCAL TEMPORARY COLUMN TABLE " + stage + " AS (SELECT " + list(columns, "t") + " FROM " + target
				+ " t WHERE 1=0)";
	}
