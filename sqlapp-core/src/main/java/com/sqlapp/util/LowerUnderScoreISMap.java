...
public class LowerUnderScoreISMap<T> extends AbstractStringMap<T>{
	 ...
	 */
	public LowerUnderScoreISMap(final int capacity){
		super(capacity);
	}
	 ...
	 * @param capacity マップの初期サイズ
	 */
	public LowerUnderScoreISMap(final int capacity, final float loadFactor){
	 ...
	 */
	public LowerUnderScoreISMap(final Map<String, T> baseMap){
		super(baseMap);
	}
	 ...
	 */
	public LowerUnderScoreISMap<T> clone(){
		LowerUnderScoreISMap<T> clone=new LowerUnderScoreISMap<T>();
	...
	protected String convertKey(final String key) {
		...
		String val= key.toLowerCase().replace("_", "");
		return val;
	}
