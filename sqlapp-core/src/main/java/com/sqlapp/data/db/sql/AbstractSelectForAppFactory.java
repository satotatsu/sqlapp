 ...
 * 
 */
public abstract class AbstractSelectForAppFactory<S extends AbstractSqlBuilder<?>> extends AbstractTableFactory<S> {
	...
	public List<SqlOperation> createSql(final Table obj) {
		...
		addSql(sqlList, builder, SqlType.SELECT_FOR_APP, obj);
		return sqlList;
	}
	...
	protected void addOrderBy(final Table obj, final S builder) {
		builder.lineBreak();
		builder._add(toIfExpression("!" + ParameterDefinition.COUNTSQL_KEY_PARANETER_NAME + " && "
				+ toIsNotEmptyExpression(ParameterDefinition.ORDER_BY_KEY_PARANETER_NAME)));
		builder.lineBreak();
	...
	protected void addOffsetRowsOnly(final Table obj, final S builder) {
		if (this.getDialect().supportsStandardOffsetFetchRows()) {
			builder.lineBreak()._add(toIfIsNotEmptyExpression("" + ParameterDefinition.OFFSET_KEY_PARANETER_NAME))
					.lineBreak();
			...
			builder.fetch().first().space()._add("/*" + ParameterDefinition.ROW_KEY_PARANETER_NAME + "*/1").space()
					.rows().only();
			builder.lineBreak();
			builder._add(this.getEndIfExpression());
		}
