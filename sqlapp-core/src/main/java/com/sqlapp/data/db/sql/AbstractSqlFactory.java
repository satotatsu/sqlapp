...
public abstract class AbstractSqlFactory<T extends DbCommonObject<?>, S extends AbstractSqlBuilder<?>>
	...
	@Override
	public List<SqlOperation> createDiffSql(final Collection<DbObjectDifference> differenceCollection) {
		final List<SqlOperation> result = CommonUtils.list();
		List<DbObjectDifference> diffrences = DbObjectDifferenceCollection.getByStates(differenceCollection,
				State.Deleted);
		if (!CommonUtils.isEmpty(diffrences)) {
			final SqlFactory<?> sqlFactory = this.getSqlFactoryRegistry().getSqlFactory(CommonUtils.first(diffrences));
			diffrences = sort(sqlFactory, diffrences);
			for (final DbObjectDifference dbObjectDifference : diffrences) {
				final List<SqlOperation> operations = sqlFactory.createDiffSql(dbObjectDifference);
				result.addAll(operations);
			}
		}
		diffrences = DbObjectDifferenceCollection.getByStates(differenceCollection, State.Added);
		if (!CommonUtils.isEmpty(diffrences)) {
			final SqlFactory<?> sqlFactory = this.getSqlFactoryRegistry().getSqlFactory(CommonUtils.first(diffrences));
			diffrences = sort(sqlFactory, diffrences);
			for (final DbObjectDifference dbObjectDifference : diffrences) {
		...
		diffrences = DbObjectDifferenceCollection.getByStates(differenceCollection, State.Modified);
		if (!CommonUtils.isEmpty(diffrences)) {
			final SqlFactory<?> sqlFactory = this.getSqlFactoryRegistry().getSqlFactory(CommonUtils.first(diffrences));
			diffrences = sort(sqlFactory, diffrences);
			for (final DbObjectDifference dbObjectDifference : diffrences) {
				final List<SqlOperation> dbOperations = sqlFactory.createDiffSql(dbObjectDifference);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sqlapp.data.db.sql.SqlFactory#createDiffSql(com.sqlapp.data.schemas.
	 * DbObjectDifference)
	 */
	@Override
	...
	protected boolean addSchemaName(final DbObject<?> object, final AbstractSqlBuilder<?> builder) {
	...
	protected void addSql(final List<SqlOperation> sqlList, final AbstractSqlBuilder<?> builder, final SqlType sqlType,
			final List<? extends DbCommonObject<?>> originals) {
		if (builder == null) {
			return;
		}
		final String sql = builder.toString();
	...
	protected SqlOperation createOperation(final String text, final SqlType sqlType,
			final List<? extends DbCommonObject<?>> originals) {
	...
	protected SqlOperation createOperation(final String text, final SqlType sqlType,
			final List<? extends DbCommonObject<?>> originals, final List<? extends DbCommonObject<?>> targets) {
	...
	private String getCoalesceValueDefinition(final Column column, final String columnDefault,
	...
	protected String getValueDefinitionForInsert(final Row row, final Column column) {
		...
		return tableOption.getInsertRowSqlValue().apply(row, column,
				this.getDialect().getSqlValueDefinition(column, value));
	}
	...
	protected String getValueDefinitionForUpdate(final Row row, final Column column) {
		...
		if (this.isAutoIncrementColumn(column)) {
			...
			return tableOption.getUpdateRowSqlValue().apply(row, column,
					this.getOptimisticLockColumnUpdateDefinition(null, column));
		} else if (isUpdatedAtColumn(column)) {
