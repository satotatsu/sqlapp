 ...
 * 
 */
public abstract class AbstractDdlAutoCommitOffFactory<S extends AbstractSqlBuilder<?>> extends AbstractSqlFactory<DbCommonObject<?>,S> {
	...
	public List<SqlOperation> createSql() {
		...
		addSql(sqlList, builder, SqlType.DDL_AUTOCOMMIT_OFF, (DbCommonObject<?>)null);
		return sqlList;
	}
	
	protected void createSqlInternal(S builder){
		builder.set().transaction().autocommit().off();
	}
