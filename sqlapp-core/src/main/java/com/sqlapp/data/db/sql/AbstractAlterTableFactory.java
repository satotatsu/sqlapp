...
public abstract class AbstractAlterTableFactory<S extends AbstractSqlBuilder<?>> extends AbstractTableFactory<S> {
	...
	public List<SqlOperation> createDiffSql(final DbObjectDifference difference) {
		...
		if (consDiff != null) {
			addConstraintDefinitions(allDiff, originalTable, table, consDiff.getList(State.Added, State.Modified),
					result);
		}
		if (indexesDiff != null) {
			addIndexDefinitions(allDiff, originalTable, table, indexesDiff.getList(State.Added, State.Modified),
					result);
		}
	...
	protected void addRenameOrAlterColumn(final Table originalTable, final Table table, final DbObjectDifference diff,
			final List<SqlOperation> result) {
		final Column oldColumn = diff.getOriginal(Column.class);
		final Column column = diff.getTarget(Column.class);
		final Map<String, Difference<?>> colDiff = diff.toDifference().getProperties(this.getDialect(), State.Added,
				State.Modified);
	 ...
	 * @param result
	 */
	protected void addIndexDefinitions(final Map<String, Difference<?>> allDiff, final Table originalTable,
			final Table table, final List<DbObjectDifference> indexesDiff, final List<SqlOperation> result) {
	 ...
	 * @param result
	 */
	protected void addIndexDefinition(final Table originalTable, final Table table, final Index originalIndex,
	...
	protected void addCreateIndexDefinition(final Table originalTable, final Table table, final Index originalIndex,
			final Index index, final DbObjectDifference diff, final List<SqlOperation> result) {
		final S builder = createSqlBuilder();
	 ...
	 * @param result
	 */
	protected void addConstraintDefinitions(final Map<String, Difference<?>> allDiff, final Table originalTable,
			final Table table, final List<DbObjectDifference> consDiff, final List<SqlOperation> result) {
	...
	protected void addConstraintDefinition(final Table originalTable, final Table table, final Constraint obj,
			final S builder) {
		if (obj == null) {
			return;
		}
	...
	protected void addConstraintDefinition(final Table table, final Constraint constraint,
			final AddTableObjectDetailFactory<Constraint, AbstractSqlBuilder<?>> sqlFactory, final S builder) {
		builder.alter().table();
		builder.name(table, this.getOptions().isDecorateSchemaName());
		builder.add();
	...
	protected void dropConstraintDefinition(final Constraint obj, final S builder) {
		builder.alter().table().name(obj.getParent().getTable(), this.getOptions().isDecorateSchemaName()).drop()
				.constraint();
