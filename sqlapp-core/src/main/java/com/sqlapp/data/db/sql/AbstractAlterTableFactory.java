...
public abstract class AbstractAlterTableFactory<S extends AbstractSqlBuilder<?>>
	...
	public List<SqlOperation> createDiffSql(DbObjectDifference difference) {
		...
		DbObjectDifferenceCollection indexesDiff = (DbObjectDifferenceCollection) allDiff
	...
	protected void addRenameOrAlterColumn(Table originalTable, Table table, DbObjectDifference diff,List<SqlOperation> result){
		...
		Column column = diff.getTarget(Column.class);
		Map<String, Difference<?>> colDiff = diff.toDifference()
	 ...
	 * @param result
	 */
	protected void addIndexDefinitions(Map<String, Difference<?>> allDiff
			, Table originalTable, Table table,
			List<DbObjectDifference> indexesDiff, List<SqlOperation> result) {
	 ...
	 * @param result
	 */
	protected void addIndexDefinition(Table originalTable, Table table, Index originalIndex, Index index,
	 ...
	 * @param result
	 */
	protected void addConstraintDefinition(Table originalTable, Table table,
			Constraint originalConstraint,Constraint constraint,
			DbObjectDifference diff, List<SqlOperation> result) {
	 ...
	 * @param result
	 */
	protected void addCreateConstraintDefinition(Table originalTable, Table table,
	 ...
	 * @param result
	 */
	protected void addDropConstraintDefinition(Table originalTable, Table table,
			Constraint originalConstraint,Constraint constraint,
			DbObjectDifference diff, List<SqlOperation> result) {
		S builder = createSqlBuilder();
	...
	protected void dropConstraintDefinition(Constraint obj,
			S builder) {
