...
public class JdbcFunctionArgumentReader extends RoutineArgumentReader<Function> {

	public JdbcFunctionArgumentReader(Dialect dialect) {
		super(dialect);
	}
	...
	protected List<NamedArgument> doGetAll(Connection connection,
		...
		try {
			...
			rs = new ExResultSet(databaseMetaData.getFunctionColumns(CommonUtils.coalesce(emptyToNull(this.getCatalogName(context)), emptyToNull(this.getCatalogName()))
