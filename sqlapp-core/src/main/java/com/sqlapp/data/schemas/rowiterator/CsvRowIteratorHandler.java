...
public class CsvRowIteratorHandler extends AbstractRowIteratorHandler{
	...
	public Iterator<Row> iterator(final RowCollection c) {
		if (file!=null){
			return new CsvRowIterator(c, file, workbookFileType, charset, 0L, this.getRowValueConverter());
		}else if (path!=null){
			return new CsvRowIterator(c, path, workbookFileType, charset, 0L, this.getRowValueConverter());
		} else{
			return new CsvRowIterator(c, reader, workbookFileType, 0L, this.getRowValueConverter());
		}
	}
	...
	public ListIterator<Row> listIterator(final RowCollection c, final int index) {
		if (file!=null){
			return new CsvRowIterator(c, file, workbookFileType, charset, index, this.getRowValueConverter());
		}else if (path!=null){
			return new CsvRowIterator(c, path, workbookFileType, charset, index, this.getRowValueConverter());
		} else{
			return new CsvRowIterator(c, reader, workbookFileType, index, this.getRowValueConverter());
		}
	}
	...
	public static class CsvRowIterator extends AbstractTextRowListIterator<List<String>> {
		CsvRowIterator(final RowCollection c, final Path path, final WorkbookFileType workbookFileType, final String charset, final long index, final RowValueConverter valueConverter){
		...
		CsvRowIterator(final RowCollection c, final File file, final WorkbookFileType workbookFileType, final String charset, final long index, final RowValueConverter valueConverter){
			super(c, index, valueConverter);
			this.workbookFileType=workbookFileType;
			this.file=file;
			this.path=null;
		...
		CsvRowIterator(final RowCollection c, final Reader reader, final WorkbookFileType workbookFileType, final long index, final RowValueConverter valueConverter){
			super(c, index, valueConverter);
			this.workbookFileType=workbookFileType;
			this.file=null;
			this.path=null;
