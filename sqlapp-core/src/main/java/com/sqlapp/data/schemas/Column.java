 ...
 * 
 */
public final class Column extends AbstractColumn<Column>
		implements HasParent<ColumnCollection>, TableNameProperty<Column>, HiddenProperty<Column>,
		OnUpdateProperty<Column>, PrimaryKeyGetter, CheckProperty<Column>, MaskingFunctionProperty<Column> {
	...
	public Column setCheck(final String check) {
		if (check != null) {
			if (this.checkConstraint != null) {
				this.checkConstraint.setExpression(check);
			} else {
				this.checkConstraint = new CheckConstraint(this);
				this.checkConstraint.setExpression(check);
			}
		} else {
		...
		return new AbstractNamedObjectXmlReaderHandler<Column>(this.newInstance()) {
			...
			protected void initializeSetValue() {
				...
				register(keyName, new AbstractSetValue<Column, Map<String, Object>>() {
					@Override
					public void setValue(final Column target, final String name, final Map<String, Object> setValue)
							throws XMLStreamException {
						target.setExtendedProperties(setValue);
					}
				});
				registerTransparent(keyName, new MapHandler(), new EmptyTextSkipHandler());
			}
	...
	public Column setName(final String name) {
		...
		if (table != null) {
			...
			final List<ReferenceColumnCollection> childColumnsList = table.getChildRelations().stream()
			...
			final List<ReferenceColumnCollection> ucColumnsList = table.getConstraints().getUniqueConstraints().stream()
					.filter(obj -> {
						for (final ReferenceColumn refColumn : obj.getColumns()) {
							if (CommonUtils.eq(refColumn.getName(), origianlName)) {
								return true;
			...
			// change unique constraint columns
			final List<ReferenceColumnCollection> ecColumnsList = table.getConstraints().getExcludeConstraints()
					.stream().filter(obj -> {
						for (final ReferenceColumn refColumn : obj.getColumns()) {
							if (CommonUtils.eq(refColumn.getName(), origianlName)) {
								return true;
			...
			table.getIndexes().stream().filter(obj -> obj.getPartitioning() != null).map(obj -> obj.getPartitioning())
					.forEach(p -> {
	...
	private void changeReferenceColumnName(final String originalName, final String name,
			final List<ReferenceColumnCollection> columnsList) {
