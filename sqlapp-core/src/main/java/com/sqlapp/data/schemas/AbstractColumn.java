 ...
 * 
 */
public abstract class AbstractColumn<T extends AbstractColumn<T>> extends AbstractSchemaObject<T>
		implements DataTypeSetProperties<T>, NotNullProperty<T>, ValuesProperty<T>, DefaultValueProperty<T>,
	...
	private CharacterSemantics characterSemantics = (CharacterSemantics) SchemaProperties.CHARACTER_SEMANTICS
			.getDefaultValue();
	/** characterSetName */
	@SuppressWarnings("unused")
	...
	/** 自動採番生成タイプ */
	private IdentityGenerationType identityGenerationType = (IdentityGenerationType) SchemaProperties.IDENTITY_GENERATION_TYPE
			.getDefaultValue();
	/** シーケンス */
	@SuppressWarnings("unused")
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sqlapp.data.schemas.StringUnits#setStringUnits(java.lang.String)
	 */
	@Override
	...
	public boolean equals(Object obj, EqualsHandler equalsHandler) {
		...
		if (!CommonUtils.eq(this.getSequenceSchemaName(), this.getSchemaName())
				|| !CommonUtils.eq(val.getSequenceSchemaName(), val.getSchemaName())) {
		...
		if (!equals(SchemaProperties.STRING_UNITS, val, equalsHandler,
				EqualsUtils.getEqualsIgnoreCaseSupplier(this.getStringUnits(), val.getStringUnits()))) {
			return false;
		}
		return true;
	}
	...
	protected void writeXmlOptionalAttributes(StaxWriter stax) throws XMLStreamException {
		...
		if (this.isIdentity()) {
			...
			stax.writeAttribute(SchemaProperties.IDENTITY_START_VALUE.getLabel(), this.getIdentityStartValue());
			stax.writeAttribute(SchemaProperties.IDENTITY_MAX_VALUE.getLabel(), this.getIdentityMaxValue());
			stax.writeAttribute(SchemaProperties.IDENTITY_MIN_VALUE.getLabel(), this.getIdentityMinValue());
			stax.writeAttribute(SchemaProperties.IDENTITY_STEP.getLabel(), this.getIdentityStep());
			stax.writeAttribute(SchemaProperties.IDENTITY_LAST_VALUE.getLabel(), this.getIdentityLastValue());
			...
			if (this.isIdentityCache() != DEFAULT_VALUE_SEQUENCE.isCache()) {
				stax.writeAttribute(SchemaProperties.IDENTITY_CACHE.getLabel(), this.isIdentityCache());
			}
			stax.writeAttribute(SchemaProperties.IDENTITY_CACHE_SIZE.getLabel(), this.getIdentityCacheSize());
			if (this.isIdentityCycle() != DEFAULT_VALUE_SEQUENCE.isCycle()) {
				stax.writeAttribute(SchemaProperties.IDENTITY_CYCLE.getLabel(), this.isIdentityCycle());
			}
			if (this.isIdentityOrder() != DEFAULT_VALUE_SEQUENCE.isOrder()) {
				stax.writeAttribute(SchemaProperties.IDENTITY_ORDER.getLabel(), this.isIdentityOrder());
			}
		} else {
		...
		if (!isEmpty(this.getFormula())) {
			if (this.isFormulaPersisted()) {
				stax.writeAttribute(SchemaProperties.FORMULA_PERSISTED.getLabel(), this.isFormulaPersisted());
			}
		}
	}
	...
	/**
	 * @param arrayDimension the arrayDimension to set
	 */
	@Override
