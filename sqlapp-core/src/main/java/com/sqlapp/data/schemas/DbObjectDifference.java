...
public class DbObjectDifference extends AbstractDifference<DbObject<?>> {
	...
	protected DbObjectDifference(String propertyName, DbObject<?> original, DbObject<?> target,
	...
	public DbObjectDifference(DbObject<?> original, DbObject<?> target, EqualsHandler equalsHandler) {
		this(null, original, target, equalsHandler, false);
	}

	protected DbObjectDifference(final DbCommonObject<?> originalParent, final DbObject<?> original,
			final DbCommonObject<?> targetParent, final DbObject<?> target, final EqualsHandler equalsHandler,
			boolean skipDiff) {
	...
	protected DbObjectDifference(String propertyName, final DbCommonObject<?> originalParent,
			final DbObject<?> original, final DbCommonObject<?> targetParent, final DbObject<?> target,
	...
	public DbObjectDifference reverse() {
		DbObjectDifference reverse = new DbObjectDifference(this.getPropertyName(), this.getTargetParent(),
				this.getTarget(), this.getOriginalParent(), this.getOriginal(), this.getEqualsHandler(), false);
		return reverse;
	}
	...
	protected void diff() {
		...
		if (this.getOriginal() == null || this.getTarget() == null) {
			...
			((DbCommonObject<?>) this.getOriginal()).equals(this.getTarget(),
					new DifferenceEqualsHandler(this, this.getEqualsHandler()));
		...
		for (Map.Entry<String, Difference<?>> entry : properties.entrySet()) {
			if (entry.getValue().getState().isChanged()) {
				if (entry.getValue() instanceof DbObjectPropertyDifference) {
					if (SchemaProperties.SPECIFICS.getLabel().equals(entry.getKey())
							|| SchemaProperties.STATISTICS.getLabel().equals(entry.getKey())) {
						List<DbObjectPropertyDifference> props = ((DbObjectPropertyDifference) entry.getValue())
								.toList();
	 ...
	 * @param clazz
	 */
	public List<DbObjectPropertyDifference> findModifiedProperties(Dialect dialect, String propertyName,
		...
		if (isTargetClass(this.getOriginal(), clazz) || isTargetClass(this.getTarget(), clazz)) {
			...
			for (Map.Entry<String, Difference<?>> entry : differeces.entrySet()) {
				if (entry.getValue() instanceof DbObjectDifferenceCollection) {
					...
					List<DbObjectPropertyDifference> childResult = diffCollection.findModifiedProperties(dialect,
							propertyName, clazz);
	...
	protected static class DifferenceEqualsHandler extends EqualsHandler {
		...
		protected boolean equalsInternal(String propertyName, boolean result, Object object1, Object object2,
			...
			if (isDbObjectCollection(value1, value2)) {
				DbObjectDifferenceCollection diff = new DbObjectDifferenceCollection(propertyName,
						(DbObjectCollection<?>) value1, (DbObjectCollection<?>) value2, equalsHandler, result);
				...
				DbObjectDifference diff = new DbObjectDifference(propertyName, (DbCommonObject<?>) object1,
						(DbObject<?>) value1, (DbCommonObject<?>) object2, (DbObject<?>) value2, equalsHandler, result);
				diff.setParentDifference(dbObjectDifference);
				dbObjectDifference.getProperties().put(propertyName, diff);
			} else {
				DbObjectPropertyDifference diff = new DbObjectPropertyDifference(propertyName,
						(DbCommonObject<?>) object1, value1, (DbCommonObject<?>) object2, value2, equalsHandler,
						result);
				diff.setParentDifference(dbObjectDifference);
				if (result) {
...
//		} else {
		sepBuilder.setStart(getStateText(this.getState()) + ":" + this.getSimpleName(this.getOriginal()));
//		}
