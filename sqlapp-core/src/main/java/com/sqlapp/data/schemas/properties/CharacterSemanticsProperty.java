...
public interface CharacterSemanticsProperty<T> {
	...
	default CharacterSemantics getCharacterSemantics(){
		...
		CharacterSemantics value= SimpleBeanUtils.getField(this, SchemaProperties.CHARACTER_SEMANTICS.getLabel());
		if (value==null){
			if (this instanceof DbCommonObject){
				DbCommonObject<?> dbObj=(DbCommonObject<?>)this;
				return SchemaUtils.getParentCharacterSemantics(dbObj);
			}
		}
		return value;
	}

	T setCharacterSemantics(CharacterSemantics value);

	default T setCharacterSemantics(String value){
		return setCharacterSemantics(CharacterSemantics.parse(value));
	}
}
