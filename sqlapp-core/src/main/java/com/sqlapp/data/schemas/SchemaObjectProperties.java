...
public enum SchemaObjectProperties implements ISchemaProperty {
	...
	COLUMN_ARRAY("columns") {
		...
		public List<Column> getCloneValue(final Object obj) {
			...
			final List<Column> clone = CommonUtils.list(value.size());
			for (int i = 0; i < value.size(); i++) {
				if (value.get(i) != null) {
					clone.add(value.get(i).clone());
				}
			}
			return clone;
		}
		...
		protected final boolean setValueInternal(final Object obj, final Object value) {
			...
			if (value instanceof Column[]) {
				...
				cst.setColumns((List<Column>) value);
			} else if (value instanceof Collection) {
				List<Column> columns = CommonUtils.list((Collection<Column>) value);
				cst.setColumns(columns);
				return true;
			}
			return false;
		}
