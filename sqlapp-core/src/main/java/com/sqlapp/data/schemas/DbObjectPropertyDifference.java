...
public class DbObjectPropertyDifference extends AbstractDifference<Object> {
	...
	protected DbObjectPropertyDifference(String propertyName,
			DbCommonObject<?> original, DbCommonObject<?> target) {
		this(propertyName, original, target, new EqualsHandler());
	}

	protected DbObjectPropertyDifference(DbCommonObject<?> original,
			DbCommonObject<?> target) {
	...
	protected DbObjectPropertyDifference(
			final DbCommonObject<?> originalParent, final Object original,
			final DbCommonObject<?> targetParent, final Object target,
	...
	protected DbObjectPropertyDifference(
		...
		this(null, originalParent, original, targetParent, target,
				equalsHandler, true);
	}
	...
	protected static class DifferenceEqualsHandler extends EqualsHandler {
		...
		protected DifferenceEqualsHandler(
				DbObjectPropertyDifference dbObjectDifference,
				EqualsHandler equalsHandler) {
		...
		protected boolean equalsInternal(String propertyName, boolean result,
			...
			DbObjectPropertyDifference diff = new DbObjectPropertyDifference(
					...
					(DbCommonObject<?>) object2, value2, equalsHandler, result);
	...
	protected void toString(ToStringBuilder builder) {
		...
		} else if (this.getOriginalParent() instanceof Row&&this.getTargetParent() instanceof Row &&Row.VALUES.equals(this.getPropertyName())){
	...
	protected List<DbObjectPropertyDifference> toList(){
		...
		if (this.getOriginal() instanceof DbInfo||this.getTarget() instanceof DbInfo){
			...
			for(Map.Entry<String,DiffValue> entry:diffValueMap.entrySet()){
				...
				DbObjectPropertyDifference diff=new DbObjectPropertyDifference(entry.getKey(), this.getOriginalParent()
						, diffValue.getOriginal(), this.getTargetParent(), diffValue.getTarget(), true);
				if (reverse){
	...
	private void buildDiff(ToStringBuilder builder, List val1, List val2){
		...
		for (Delta<?> delta : patch.getDeltas()) {
			...
			if (isString){
				bld.append(toTabString(
						DeltaUtils.toStringLineNumber(delta,
								max), space));
			} else{
				bld.append(toTabString(
						DeltaUtils.toStringLine(delta), space));
			}
		}
		bld.append("\n");
		bld.append(space);
	...
	private void buildDiff(ToStringBuilder builder, Object val1, Object val2){
		if (val1!=null&&val2!=null&&val1.getClass().isArray()&&val2.getClass().isArray()){
			...
			builder.add(this.getPropertyName(), "(" + this.getOriginal()
			+ " -> " + this.getTarget() + ")");
		}
	}
	...
	private void buildDiff(ToStringBuilder builder, byte[] val1, byte[] val2){
		...
		builder.add(this.getPropertyName()+".length", "(" + length((byte[])this.getOriginal())
		+ " -> " + length((byte[])this.getTarget()) + ")");
	}
	...
	public int compareTo(Difference<?> o) {
		...
		if (this.getOriginal() instanceof List<?>
				|| this.getTarget() instanceof List<?>) {
			if (dop.getOriginal() instanceof List<?>
					|| dop.getTarget() instanceof List<?>) {
				return 0;
			} else {
				return 1;
			}
