...
class PaddingTypeTest {
	...
	void testToStringRIGHT2() {
		...
		final String padText = " \t";
		final Charset charset = Charset.forName("MS932");
		final String bytes = PaddingType.RIGHT.toString(text.getBytes(charset), 0, text.getBytes(charset).length,
				padText.getBytes(charset), charset);
		assertEquals("", bytes);
	}
	...
	void testToStringLEFT1() {
		...
		final String padText = " \t";
		final Charset charset = Charset.forName("MS932");
		final String bytes = PaddingType.LEFT.toString(text.getBytes(charset), 0, text.getBytes(charset).length,
				padText.getBytes(charset), charset);
		assertEquals("あいう \t \t", bytes);
	}
