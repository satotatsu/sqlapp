...
public class PostgresUnicodeFastCollationBuilder {
	...
	public String create(String schemaName, String name, boolean ifNotExists) {
		checkVersion();
		...
		return builder.append(" (PROVIDER = builtin, LOCALE = 'PG_UNICODE_FAST')").toString();
	}

	public String drop(String schemaName, String name, boolean ifExists, boolean cascade) {
		checkVersion();
	...
	private void appendName(StringBuilder builder, String schemaName, String name) {
	...
	private void checkVersion() {
		if (dialect.compareTo(DialectHolder.postgreSQL180) < 0) {
			throw new IllegalArgumentException("PG_UNICODE_FAST requires PostgreSQL 18 or later.");
		}
	}
