...
public class PostgresCreateOperatorFactory extends AbstractCreateOperatorFactory<PostgresSqlBuilder> {
	...
	protected void addCreateObject(final Operator obj, PostgresSqlBuilder builder) {
		...
		if (obj.getCommutativeOperator() != null) {
			...
			if (!CommonUtils.eq(obj.getSchemaName(), obj.getCommutativeOperator().getSchemaName())
					&& !CommonUtils.isEmpty(obj.getCommutativeOperator().getSchemaName())) {
		...
		if (obj.getNegationOperator() != null) {
			builder.lineBreak();
			builder.negator().eq();
			if (!CommonUtils.eq(obj.getSchemaName(), obj.getNegationOperator().getSchemaName())
					&& !CommonUtils.isEmpty(obj.getNegationOperator().getSchemaName())) {
		...
		if (obj.getRestrictFunction() != null) {
			builder.lineBreak();
			builder.restrict().eq();
			if (!CommonUtils.eq(obj.getSchemaName(), obj.getRestrictFunction().getSchemaName())
					&& !CommonUtils.isEmpty(obj.getRestrictFunction().getSchemaName())) {
		...
		if (obj.getJoinFunction() != null) {
			builder.lineBreak();
			builder.join().eq();
			if (!CommonUtils.eq(obj.getSchemaName(), obj.getJoinFunction().getSchemaName())
					&& !CommonUtils.isEmpty(obj.getJoinFunction().getSchemaName())) {
