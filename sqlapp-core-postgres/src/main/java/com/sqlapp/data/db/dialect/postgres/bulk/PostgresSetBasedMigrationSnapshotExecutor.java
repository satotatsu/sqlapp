...
/**
 * PostgreSQL-specific temporary-table syntax for the shared set-based SCD2
 * flow.
 */
public final class PostgresSetBasedMigrationSnapshotExecutor extends AbstractStagingMigrationSnapshotExecutor {
	public PostgresSetBasedMigrationSnapshotExecutor(final Dialect dialect) {
		super(dialect);
	}

	@Override
	protected String createStageSql(final String stage, final String target, final List<String> columns) {
		return "CREATE TEMPORARY TABLE " + stage + " ON COMMIT DROP AS SELECT " + list(columns, "t") + " FROM " + target
				+ " t WITH NO DATA";
	}
