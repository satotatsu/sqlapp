...
public class Postgres180CreateForeignKeyConstraintFactory
	...
	@Override
	public void addObjectDetail(ForeignKeyConstraint obj, Table table, AbstractSqlBuilder<?> builder) {
		boolean temporal = Boolean.parseBoolean(obj.getSpecifics().get(PERIOD));
		if (temporal && obj.getColumns().isEmpty()) {
			throw new IllegalArgumentException("Temporal foreign key requires a PERIOD column.");
		}
	...
	private void addLocalColumns(AbstractSqlBuilder<?> builder, ForeignKeyConstraint obj, boolean temporal) {
	...
	private void addRelatedColumns(AbstractSqlBuilder<?> builder, ForeignKeyConstraint obj, boolean temporal) {
	...
	@Override
	protected void addAfter(ForeignKeyConstraint constraint, AbstractSqlBuilder<?> builder) {
