...
public class PostgresBulkInsertExecutor implements BulkInsertExecutor {
	...
	public long execute(final Connection connection, final Table table, final BulkOption options) throws SQLException {
		...
		try (PostgresBulkDataReader reader = new PostgresBulkDataReader(table, options)) {
			final PostgresCopyFromBuilder builder = new PostgresCopyFromBuilder(dialect, table.getSchemaName(),
					table.getName()).format("csv");
			...
			throw new SQLException("Failed to stream PostgreSQL COPY rows for " + table.getName(), e);
		}
	}
	...
	private void validateOptions(final BulkOption options) {
		...
		if (options.getBatchSize() != null || options.getBulkCopyTimeout() != null
