...
final class Postgres180ColumnMetadata {
	...
	static void applyNamedNotNull(Column column, String constraintName, boolean noInherit, boolean validated) {
		if (!CommonUtils.isEmpty(constraintName)) {
			column.getSpecifics().put(CONSTRAINT_NAME, constraintName);
			column.getSpecifics().put(NO_INHERIT, Boolean.toString(noInherit));
			column.getSpecifics().put(VALIDATED, Boolean.toString(validated));
		}
	}
	...
	static void moveNamedNotNullConstraints(Table table) {
		for (Column column : table.getColumns()) {
			...
			boolean noInherit = Boolean.parseBoolean(column.getSpecifics().remove(NO_INHERIT));
			boolean validated = Boolean.parseBoolean(column.getSpecifics().remove(VALIDATED));
			NotNullConstraint constraint = new NotNullConstraint(name, column).setNoInherit(noInherit)
