...
class SqlServerBulkInsertTest {
	...
	void insertsSchemaRowsUsingMicrosoftJdbcBulkCopy() throws Exception {
		try (Connection connection = DriverManager.getConnection(SQL_SERVER.getJdbcUrl(), SQL_SERVER.getUsername(),
			...
			assertEquals(2, BulkInsertResolver.execute(connection, table,
					BulkOption.builder().batchSize(1).keepNulls(true).checkConstraints(true).tableLock(true).build()));
			try (var resultSet = statement
					.executeQuery("SELECT ID, NAME, AMOUNT FROM dbo.SQLAPP_BULK_TEST ORDER BY ID")) {
				resultSet.next();
	...
	void preservesExplicitIdentityWhenRequested() throws Exception {
		try (Connection connection = DriverManager.getConnection(SQL_SERVER.getJdbcUrl(), SQL_SERVER.getUsername(),
				SQL_SERVER.getPassword()); var statement = connection.createStatement()) {
			statement.execute(
					"CREATE TABLE dbo.SQLAPP_BULK_IDENTITY_TEST (ID BIGINT IDENTITY(1,1) PRIMARY KEY, NAME NVARCHAR(100))");
			...
			try (var resultSet = statement.executeQuery("SELECT ID, NAME FROM dbo.SQLAPP_BULK_IDENTITY_TEST")) {
				resultSet.next();
