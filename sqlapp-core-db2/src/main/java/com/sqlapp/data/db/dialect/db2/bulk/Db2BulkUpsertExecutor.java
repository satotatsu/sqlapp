...
public class Db2BulkUpsertExecutor implements BulkUpsertExecutor {
		...
		try (var scope = BulkUpsertExecutionScope.begin(connection, option.isUseTransaction())) {
			try {
				try (var statement = connection.createStatement()) {
					statement.execute("DECLARE GLOBAL TEMPORARY TABLE " + stageSql + " AS (SELECT "
							+ columnList(staged, null) + " FROM " + target
							+ " WHERE 1 = 0) WITH NO DATA ON COMMIT PRESERVE ROWS NOT LOGGED");
				}
				scope.addCleanupSql("DROP TABLE " + stageSql);
				final Table stagingTable = plan.createStagingTable(stage).setSchemaName("SESSION");
	...
	private String mergeSql(final String target, final String stage, final List<Column> keys, final List<Column> staged,
		...
		if (option.isInsertWhenNotMatched()) {
			sql.append(" WHEN NOT MATCHED THEN INSERT (").append(columnList(staged, null)).append(") VALUES (")
					.append(columnList(staged, "source")).append(')');
		}
		return sql.toString();
	}
