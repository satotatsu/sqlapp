...
public class Db2BulkUpsertExecutor implements BulkUpsertExecutor {
	...
	private List<Column> keys(final Table table, final BulkUpsertOption option) {
		...
		if (names.isEmpty()) {
			if (table.getPrimaryKeyConstraint() == null
				...
				throw new IllegalArgumentException("Bulk upsert requires keyColumns or a primary key: "
						+ table.getName());
			}
		...
		if (columns.stream().anyMatch(Column::isIdentity)
				&& !option.getBulkOption().isKeepIdentity()) {
			throw new IllegalArgumentException("An identity key requires bulkOption.keepIdentity=true");
		}
		return columns;
	}

	private List<Column> stagedColumns(final Table table, final BulkUpsertOption option,
			final List<Column> keys) {
		...
		for (final Column column : table.getColumns()) {
			if (!column.isHidden() && CommonUtils.isEmpty(column.getFormula())
					&& (!column.isIdentity() || option.getBulkOption().isKeepIdentity()
	...
	private List<Column> updateColumns(final Table table, final BulkUpsertOption option,
			final List<Column> keys, final List<Column> staged) {
		final Set<String> keyNames = names(keys);
		final Set<String> stagedNames = names(staged);
		if (!option.getUpdateColumns().isEmpty()) {
			...
			for (final Column column : result) {
				if (keyNames.contains(column.getName()) || column.isIdentity()
					...
					throw new IllegalArgumentException("Invalid bulk upsert update column: "
							+ column.getName());
				}
			}
			return result;
		...
		for (final Column column : staged) {
			if (!keyNames.contains(column.getName()) && !column.isIdentity()) result.add(column);
		}
		return result;
	}
