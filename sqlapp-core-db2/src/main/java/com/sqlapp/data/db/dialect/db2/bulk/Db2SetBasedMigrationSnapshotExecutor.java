...
public final class Db2SetBasedMigrationSnapshotExecutor extends AbstractStagingMigrationSnapshotExecutor {
	public Db2SetBasedMigrationSnapshotExecutor(final Dialect dialect) { super(dialect); }
	@Override protected String stageIdentifier(final String name) { return "SESSION." + dialect.quote(name); }
	@Override protected String createStageSql(final String stage, final String target, final List<String> columns) {
		return "DECLARE GLOBAL TEMPORARY TABLE " + stage + " AS (SELECT " + list(columns, "t") + " FROM "
				+ target + " t) WITH NO DATA ON COMMIT PRESERVE ROWS NOT LOGGED";
	}
